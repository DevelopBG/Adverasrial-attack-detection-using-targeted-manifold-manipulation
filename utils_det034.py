...
def fake_ent(it,ent_thre,path, name = ''):
    loaded_list = torch.load(path + f'/{name}' + '.pt')
    detect = []
    k = it
    c,c1 = 0,0
...
def entropy_measure(model,dataloader_val,temperature,upper_limit,save_path,save = True):
    ...
    if save:
        ...
        for idx, (images0, labels0) in enumerate(dataloader_val):
    ...
    # print(" Searching Entroopy threshold....", end = '\r')

    position_ent = len(clean_entropy_sort) * upper_limit
    position_eng = len(clean_entropy_sort) * (1-0.995)
    # print(len(clean_entropy_sort))
    # print(int(np.floor(position_ent)))
    # print(int(np.ceil(position_ent)))
    ...
    # entropy_threshold = 0 #clean_entropy_sort[-1] /200
    # energy_threshold = clean_energy_sort[0]
