...
class RayS(object):
    def __init__(self, model, epsilon=0.031, order=np.inf,num_classes = 10):
        self.model = model
        self.ord = order
    ...
    def attack_hard_label(self, x, y, target=None, query_limit=10000, seed=None):
        """ Attack the original image and return adversarial example
            ...
            (x, y): original image
        """
        # target = get_target_labels(target=target,num_classes=self.num_classes,labels=y)
        shape = list(x.shape)
        dim = np.prod(shape[1:])
        if seed is not None:
            np.random.seed(seed)
    ...
    def search_succ(self, x, y, target, mask):
        ...
        if target:
            ...
            return self.model(x[mask]).max(1)[1] == target[mask]
        else:
            # return self.model.predict_label(x[mask]) != y[mask]
            return self.model(x[mask]).max(1)[1] != y[mask]
