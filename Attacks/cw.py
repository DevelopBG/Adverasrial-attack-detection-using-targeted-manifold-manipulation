...
def CW(model,
    ...
    def carlini_wagner_l2(
        ...
        :param model_fn: model
        :param x: input tensor of shape (n_examples, ...),
        ...
        :param lr: (optional) float. The learning rate
    ...
    if targeted:
        label_cw = get_target_labels(target,num_classes,labels)
    else:
        label_cw = labels
    img1 = carlini_wagner_l2(model_fn=model, x=images, n_classes=num_classes + 1,
                             y=label_cw, max_iterations=steps, lr=alpha, targeted=targeted,confidence=confidence,
                             clip_min=clip_min,clip_max=clip_max,initial_const=initial_const,binary_search_steps=binary_search_steps)
    return img1
